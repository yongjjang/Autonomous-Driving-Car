...
class Follower:
  ...
  def image_callback(self, msg):
    ...
    image = cv2.warpPerspective(img,M,(img_size[0],img_size[1]))#img_size
    
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    lower_yellow = numpy.array([ 0,200,200 ])
    upper_yellow = numpy.array([30, 240, 240])

    lower_white = numpy.array([210,210,210], dtype= "uint8")
    ...
    if M['m00'] > 0:
      ...
      self.twist.angular.z = (-float(err) / 100)*2.0 + ((err - perr)/(rospy.get_time() - ptime))*1/50/100 #+ (serr*dt)*1/20/100 #1 is best, starting 3 unstable
      serr = err + serr
      perr = err
